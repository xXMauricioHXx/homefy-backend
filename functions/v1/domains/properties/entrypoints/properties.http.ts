...
export const listProperties = onRequest(
...
export const scrapProperty = onRequest(
  coreContainer.cloudFunction.defaultConfig(),
  async (req, res) => {
    if (req.method !== "POST") {
      res.status(405).json({ error: "Method not allowed" });
    ...
    try {
      const property = await container.scrapPropertyUseCase.execute(
        req.body.url,
      );
      res.status(200).json(property);
    } catch (error) {
...
export const createPdfRendition = onRequest(
  coreContainer.cloudFunction.defaultConfig(),
  async (req, res) => {
    if (req.method !== "POST") {
      res.status(405).json({ error: "Method not allowed" });
